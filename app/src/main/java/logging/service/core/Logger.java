...
import logging.service.models.LogFormat;
import logging.service.models.LogLevel;
...
public class Logger {
    ...
    public void log(LogObject log){
        if(log != null && log.getFormat() != null && log.getLevel() != null && log.getRow() != null){
            if(log.getFormat() == LogFormat.CSV) {
                logCSV(log.getTime(), log.getService(), log.getLevel(), log.getRow());
            }else if(log.getFormat() == LogFormat.JSON){
                logJSON(log.getTime(), log.getService(), log.getLevel(), log.getRow());
            }
        }
    }

    private void logCSV(String time, String service, LogLevel level, Map<String, String> row){
        try {

            row.put("time", time);
            row.put("service", service);

            File fileHandler = getFileHandler(service, level, LogFormat.CSV);
    ...
    private void logJSON(String time, String service, LogLevel level, Map<String, String> row){
        try {
            row.put("time", time);
            row.put("service", service);

            FileWriter writer = new FileWriter(getFileHandler(service, level, LogFormat.JSON), true);
    ...
    private String getFilePath(String service, LogLevel level, LogFormat format, int index){
        String filePath = "logs/"+service.toLowerCase()+"/"+level.toString().toLowerCase()+"-"+index;
    ...
    private void createFile(String service, LogLevel level, LogFormat format, int index){
        try{
            File file = new File("logs");
            if(!file.exists() || !file.isDirectory()){
                file.mkdir();
            }
            file = new File("logs/"+service);
            if(!file.exists() || !file.isDirectory()){
                file.mkdir();
            }
            file = new File(getFilePath(service, level, format, index));
    ...
    private File getFileHandler(String service, LogLevel level, LogFormat format){
        int index = 0;
        File file = null;
        boolean runLoop = true;
        while(runLoop){
            String filePath = getFilePath(service, level, format, index++);
            ...
            if(file.exists() && file.isFile()){
                ...
                createFile(service, level, format, index);
            }
        }
        return file;
    }
